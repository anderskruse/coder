...
class SearchReplaceApprovalWidget(ToolApprovalWidget[SearchReplaceArgs]):
    def compose(self) -> ComposeResult:
        ...
        # Handle atomic format (old_string/new_string)
        if self.args.old_string is not None:
            # Convert atomic format to block format for display
            content = f"<<<<<<< SEARCH\n{self.args.old_string}\n=======\n{self.args.new_string or ''}\n>>>>>>> REPLACE"
            diff_lines = parse_search_replace_to_diff(content)
