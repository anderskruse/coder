...
PROJECT_DOC_FILENAMES = ["AGENTS.md", "KKCODE.md", ".kkcode.md"]
...
class SnapshotConfig(BaseSettings):
    """Configuration for snapshot/restore functionality."""

    enabled: bool = True
    include_untracked: bool = True  # Include untracked files in snapshots
    respect_gitignore: bool = True  # Don't snapshot ignored files
    size_warning_mb: int = 500  # Warn if snapshot exceeds this size (0 to disable)
    max_snapshots_per_session: int = 50
    auto_cleanup_days: int = 7  # Clean up snapshots older than N days

class EditorConfig(BaseModel):
    """Configuration for external editor integration."""
    enabled: bool = True  # Auto-enabled by default
    command: str = "code"  # "code" or "cursor"
    open_diffs: bool = True
    auto_detect: bool = True  # Auto-detect VS Code/Cursor
    diff_cleanup_delay: int = 300  # Seconds (5 minutes)
...
class ToolFormat(StrEnum):
    """Tool call format preference for different models."""
    STANDARD = "standard"  # <<<<<<< SEARCH format (Mistral, Codestral, most models)
    XML = "xml"  # <search_replace> format (Qwen models)
    ATOMIC = "atomic"  # old_string/new_string arguments (Opencode style) #### KK-code altercation
...
class ModelConfig(BaseModel):
    ...
    tool_format: ToolFormat = ToolFormat.STANDARD  # Default to atomic format
...
DEFAULT_PROVIDERS = [
    ProviderConfig(
        name="scaleway",
        api_base="https://api.scaleway.ai/configure-during-setup/v1",  # Placeholder - configured during onboarding
        api_key_env_var="SCALEWAY_API_KEY",
        backend=Backend.GENERIC,
    ),
    # #### Mistral provider (commented out) ####
    # ProviderConfig(
    #     name="mistral",
...
DEFAULT_MODELS = [
    #### KK-code altercation BEGIN - Scaleway models ####
    ModelConfig(
        name="devstral-2-123b-instruct-2512",
        provider="scaleway",
        alias="devstral",
        temperature=0.2,
        input_price=0.0,  # Scaleway pricing TBD
        output_price=0.0,
...
class VibeConfig(BaseSettings):
    ...
    #### KK-code altercation BEGIN #####
    snapshot: SnapshotConfig = Field(default_factory=SnapshotConfig)  # Snapshot/restore config
    editor: EditorConfig = Field(default_factory=EditorConfig)  # Editor integration config
    #### KK-code altercation END #####
