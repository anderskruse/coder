...
def _resolve_config_path(basename: str, type: Literal["file", "dir"]) -> Path:
    ...
    if type == "file":
        if (candidate := cwd / ".kkcode" / basename).is_file():  #### KK-code altercation
            return candidate
    elif type == "dir":
        if (candidate := cwd / ".kkcode" / basename).is_dir():  #### KK-code altercation
            return candidate
    return VIBE_HOME.path / basename
...
def resolve_local_tools_dir(dir: Path) -> Path | None:
    if not trusted_folders_manager.is_trusted(dir):
        return None
    if (candidate := dir / ".kkcode" / "tools").is_dir():  #### KK-code altercation
        return candidate
    return None
...
def resolve_local_skills_dir(dir: Path) -> Path | None:
    if not trusted_folders_manager.is_trusted(dir):
        return None
    if (candidate := dir / ".kkcode" / "skills").is_dir():  #### KK-code altercation
        return candidate
    return None
