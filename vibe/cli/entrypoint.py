...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the KK-code interactive CLI")  #### KK-code altercation
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        metavar="NAME",
        default=None,
        help="Load agent configuration from ~/.kkcode/agents/NAME.toml",  #### KK-code altercation
    )
...
def check_and_resolve_trusted_folder() -> None:
    ...
    # Check if trust check is ENABLED via environment variable
    # By default, trust check is DISABLED to avoid prompting users
    if os.environ.get("KKCODE_ENABLE_TRUST_CHECK", "").lower() in ("1", "true", "yes", "on"):
        # Only perform trust check if explicitly enabled
        if not (cwd / ".kkcode").exists() or cwd.resolve() == Path.home().resolve():  #### KK-code altercation
            return
    ...
    # If KKCODE_ENABLE_TRUST_CHECK is not set to true, skip the trust check entirely


def main() -> None:
    #### KK-code altercation BEGIN ###
    # Set terminal title
    sys.stdout.write("\033]0;KK-code\007")
    sys.stdout.flush()
