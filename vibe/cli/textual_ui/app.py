...
from vibe.cli.textual_ui.widgets.session_selector import SessionSelector  #### KK-code altercation
from vibe.cli.textual_ui.widgets.snapshot_selector import SnapshotSelector  #### KK-code altercation
...
class BottomApp(StrEnum):
    ...
    Config = auto()
    Input = auto()
    Snapshot = auto()  #### KK-code altercation
    Session = auto()  #### KK-code altercation


class KKCodeApp(App):  # noqa: PLR0904
    TITLE = "KK-code"
    ...
    def __init__(
        ...
        resumed_session_id: str | None = None,  #### KK-code altercation
        ...
        **kwargs: Any,
        ...
        self._resumed_session_id = resumed_session_id  #### KK-code altercation
        ...
        self._last_command_input: str = ""  #### KK-code altercation
    ...
    async def on_mount(self) -> None:
        import sys
        sys.stderr.write("\033]0;KK-code\007")
        sys.stderr.flush()
        ...
        # Create diff manager if enabled
        from vibe.core.editor.diff_manager import DiffManager

        diff_manager = None
        if self.config.editor.enabled and self.config.editor.open_diffs:
            diff_manager = DiffManager(
                config=self.config.editor, workdir=self.config.effective_workdir
            )

        self._diff_manager = diff_manager
    ...
    async def on_snapshot_selector_restore_requested(
        self, message: SnapshotSelector.RestoreRequested
    ) -> None:
        """Handle restore request from snapshot selector."""
        # Switch back to input first
        await self._switch_to_input_app()

        # Perform the restore based on mode
        ...
        if mode == "preview":
            ...
            file_info = f"📁 {len(snapshot.modified_files)} file(s)" if has_files else "📁 No file changes"
            files_list = ""
            if has_files:
                files_list = "\n   • " + "\n   • ".join(snapshot.modified_files[:10])
                if len(snapshot.modified_files) > 10:
                    files_list += f"\n   ... and {len(snapshot.modified_files) - 10} more"

            await self._mount_and_scroll(
                UserCommandMessage(
                    f"Preview restore to snapshot {snapshot.message_index}\n\n"
                    f"📝 Question: {snapshot.user_question.split(chr(10) + chr(10))[0]}\n"
                    f"🕐 Time: {snapshot.timestamp}\n"
                    f"{file_info}{files_list}\n\n"
                    f"Use /restore {snapshot.message_index} to apply."
                )
            )
            return

        # Set up command and call restore
        if mode == "chat":
            self._last_command_input = f"/restore {snapshot.message_index} --chat-only"
        elif mode == "files":
            self._last_command_input = f"/restore {snapshot.message_index} --code-only"
        else:  # full
            self._last_command_input = f"/restore {snapshot.message_index}"

        await self._restore_snapshot()

    async def on_snapshot_selector_cancelled(
        self, message: SnapshotSelector.Cancelled
    ) -> None:
        """Handle cancel from snapshot selector."""
        await self._switch_to_input_app()
        await self._mount_and_scroll(UserCommandMessage("Message selection cancelled."))

    async def on_session_selector_session_selected(
        self, message: SessionSelector.SessionSelected
    ) -> None:
        """Handle session selection - resume the selected session."""
        await self._switch_to_input_app()
        ...
        if not filepath:
            await self._mount_and_scroll(
                ErrorMessage("Session file not found.", collapsed=self._tools_collapsed)
            )
            return
        ...
        try:
            ...
            if self.agent:
                #### KK-code altercation BEGIN ####
                # Use the same approach as --resume: extend agent messages with non-system messages
                non_system_messages = [
                ...
                # Rebuild the UI history like when loading initial messages
                await self._rebuild_history_from_messages_with_custom_messages(messages)
            ...
            await self._mount_and_scroll(
                ErrorMessage(f"Failed to resume session: {e}", collapsed=self._tools_collapsed)
            )

    async def on_session_selector_delete_requested(
        self, message: SessionSelector.DeleteRequested
    ) -> None:
        ...
        if filepath and filepath.exists():
            try:
                filepath.unlink()
                await self._mount_and_scroll(
                    UserCommandMessage(f"Deleted session {session.get('session_id', 'unknown')[:8]}")
                )
                # Refresh the session list
                await self._show_sessions()
            except Exception as e:
                await self._mount_and_scroll(
                    ErrorMessage(f"Failed to delete session: {e}", collapsed=self._tools_collapsed)
                )
        else:
            await self._mount_and_scroll(
                ErrorMessage("Session file not found.", collapsed=self._tools_collapsed)
    ...
    async def on_session_selector_cancelled(
        self, message: SessionSelector.Cancelled
    ) -> None:
        """Handle cancel from session selector."""
        await self._switch_to_input_app()
        await self._mount_and_scroll(UserCommandMessage("Session selection cancelled."))
    #### KK-code altercation END ####
    ...
    async def _handle_command(self, user_input: str) -> bool:
        if command := self.commands.find_command(user_input):
            ...
            #### KK-code altercation BEGIN ####
            # Store the full command input for handlers that need to parse arguments
    ...
    async def _initialize_agent(self) -> None:
        ...
        try:
            agent = Agent(
                ...
                session_id=self._resumed_session_id,  #### KK-code altercation: Reuse session ID when resuming
            )

            #### KK-code altercation BEGIN #####
            if not self._current_agent_mode.auto_approve:
                # Wrap the approval callback to show preview diffs before approval
                agent.approval_callback = self._create_preview_approval_callback()
            #### KK-code altercation END #####
    ...
    #### KK-code altercation BEGIN ####
    async def _rebuild_history_from_messages_with_custom_messages(
        self, messages: list[LLMMessage]
    ) -> None:
        """Rebuild UI history from custom messages (used for session resuming)."""
        if not messages:
            return
    ...
    def _create_preview_approval_callback(self):
        ...
        return preview_approval_wrapper
    #### KK-code altercation END #####
...
"""
    ...
    #### KK-code altercation BEGIN ####
    async def _show_snapshots(self) -> None:
        """Show interactive snapshot selector in bottom panel."""
        if self.agent is None or not self.agent.snapshot_manager:
            await self._mount_and_scroll(
                ErrorMessage(
                    "Snapshots not available. Ensure snapshot feature is enabled.",
                    collapsed=self._tools_collapsed,
                )
            )
            return

        snapshots = self.agent.snapshot_manager.get_snapshots()
        if not snapshots:
            await self._mount_and_scroll(
                UserCommandMessage("No messages available for this session.")
            )
            return

        current_index = len(self.agent.messages) - 1
        await self._switch_to_snapshot_selector(snapshots, current_index)

    async def _restore_snapshot(self) -> None:
        """Restore to a previous snapshot. Usage: /restore <N> [--chat-only|--code-only|--preview]"""
        if self.agent is None or not self.agent.snapshot_manager:
            await self._mount_and_scroll(
                ErrorMessage(
        ...
        #### KK-code altercation BEGIN ####
        # Parse the message index and flags from command input
        parts = self._last_command_input.split()
        if len(parts) < 2:
            await self._mount_and_scroll(
                ErrorMessage(
                    "Usage: /restore <N> [--chat-only|--code-only|--preview]\n"
                    "Example: /restore 5\n"
                    "         /restore 5 --chat-only",
                    collapsed=self._tools_collapsed,
                )
        ...
        code_only = "--code-only" in parts
        preview_only = "--preview" in parts

        # Remove flags to get the message index
        index_parts = [p for p in parts if not p.startswith("--")]

        try:
            message_index = int(index_parts[1])
        except (ValueError, IndexError):
            await self._mount_and_scroll(
                ErrorMessage(
                    f"Invalid message index. Must be a number.\n"
                    f"Usage: /restore <N> [--chat-only|--code-only|--preview]",
                    collapsed=self._tools_collapsed,
                )
            )
            return

        # Get the snapshot for this message index
        snapshot = self.agent.snapshot_manager.get_snapshot_by_index(message_index)
        if not snapshot:
            await self._mount_and_scroll(
                ErrorMessage(
                    f"No saved data found for message {message_index}.",
                    collapsed=self._tools_collapsed,
                )
            )
            return

        # Check if snapshot has file changes
        has_file_changes = bool(snapshot.modified_files)

        # Handle preview mode
        if preview_only:
            file_info = f"📁 {len(snapshot.modified_files)} file(s)" if has_file_changes else "📁 No file changes"
            await self._mount_and_scroll(
                UserCommandMessage(
                    f"Preview restore to snapshot {message_index}\n\n"
                    f"📝 Question: {snapshot.user_question.split(chr(10) + chr(10))[0]}\n"
                    f"🕐 Time: {snapshot.timestamp}\n"
                    f"{file_info}\n"
                    f"{'   • ' + chr(10) + '   • '.join(snapshot.modified_files[:10]) if has_file_changes else ''}\n"
                    f"{'   ... and ' + str(len(snapshot.modified_files) - 10) + ' more' if len(snapshot.modified_files) > 10 else ''}\n\n"
                    f"Use /restore {message_index} to apply."
                )
            )
            return
        ...
        try:
            ...
            # Step 1: Restore files (unless --chat-only)
            if not chat_only:
                if has_file_changes:
                    # Use the proper snapshot manager restore method
                    # This will restore files from ALL snapshots up to the target
                    restore_result = await self.agent.snapshot_manager.restore_snapshot(
                        snapshot,
                        preview_only=False  # We handle preview mode separately above
                    )
                    if restore_result is None:  # Success
                        restored_count = len(snapshot.modified_files)
                    else:
                        # This shouldn't happen since we already handled preview
                        restored_count = 0

            # Step 2: Restore chat history (unless --code-only)
            deleted_snapshots = 0
            if not code_only:
                # Truncate chat history to just BEFORE the target user message.
                # message_index is a user message count (0-indexed), not a direct
                # index into self.agent.messages which interleaves system/user/
                # assistant/tool messages.
                #
                # Restoring to snapshot N means "go back to before turn N" so the
                # user can retype message N. The snapshot's files are the BEFORE
                # state of that turn, so chat must match: everything up to (but
                # not including) user message N.
                user_count = 0
                truncate_at = len(self.agent.messages)  # default: keep all
                for i, msg in enumerate(self.agent.messages):
                    if msg.role == "user":
                        if user_count == message_index:
                            truncate_at = i  # cut right before this user message
                            break
                        user_count += 1
                self.agent.messages = self.agent.messages[:truncate_at]

                # Delete snapshots after the restored point (they're now invalid)
                deleted_snapshots = self.agent.snapshot_manager.delete_snapshots_after(message_index)

            # Step 2.5: Restore plan/todo list (always, unless --code-only)
            plan_msg = ""
            if not code_only:  # Don't restore plan in code-only mode
                try:
                    todo_tool = self.agent.tool_manager.get("todo")

                    # Check if snapshot has plan data
                    if hasattr(snapshot, 'plan_todos') and snapshot.plan_todos:
                        # Convert dicts back to TodoItem objects
                        from vibe.core.tools.builtins.todo import TodoItem
                        restored_todos = [TodoItem(**todo_dict) for todo_dict in snapshot.plan_todos]
                        todo_tool.state.todos = restored_todos
                        plan_msg = f"📝 Plan restored ({len(restored_todos)} items). "
                    else:
                        # No plan data in snapshot, clear the plan
                        todo_tool.state.todos = []
                        plan_msg = "📝 Plan cleared (no plan data in snapshot). "

                    # Rebuild the todo UI area with restored plan
                    todo_area = self.query_one("#todo-area")
                    await todo_area.remove_children()

                    # Re-mount todo items if there are any
                    if todo_tool.state.todos:
                        from vibe.cli.textual_ui.widgets.tools import ToolResultMessage
                        todo_message = ToolResultMessage(
                            tool_name="todo",
                            content=f"Restored {len(todo_tool.state.todos)} todo items",
                            collapsed=self._todos_collapsed,
                        )
                        await todo_area.mount(todo_message)

                except Exception as e:
                    logger.error(f"Failed to restore plan: {e}")
                    # If todo tool is not available or restoration fails, that's fine - just continue
                    plan_msg = "📝 Plan restoration failed, cleared plan. "

                # Clear messages from UI to match the truncated chat history
                messages_area = self.query_one("#messages")
                await messages_area.remove_children()

                # Re-mount the messages up to the restored point
                for msg in self.agent.messages[1:]:  # Skip system message
                    if msg.role == "user":
                        await messages_area.mount(UserMessage(msg.content))
                    elif msg.role == "assistant":
                        assistant_msg = AssistantMessage(msg.content)
                        await messages_area.mount(assistant_msg)
                        await assistant_msg.write_initial_content()

                # Get the user's question from the snapshot (only first line before \n\n)
                user_question = snapshot.user_question.split("\n\n")[0]

                # Place the question in the input field (ready to edit/resubmit)
                if self._chat_input_container:
                    self._chat_input_container.value = user_question

            # Step 3: Show success message
            cleanup_msg = f"🗑️ {deleted_snapshots} later snapshot(s) removed\n" if deleted_snapshots > 0 else ""
            # plan_msg is set during plan restoration above

            if chat_only:
                await self._mount_and_scroll(
                    UserCommandMessage(
                        f"✅ Restored chat to snapshot {message_index}\n\n"
                        f"💬 Chat history truncated to message {message_index}\n"
                        f"{cleanup_msg}"
                        f"{plan_msg}"
                        f"📋 User question placed in input field\n\n"
                        f"Files were NOT modified (--chat-only mode)."
                    )
                )
            elif code_only:
                await self._mount_and_scroll(
                    UserCommandMessage(
                        f"✅ Restored code to snapshot {message_index}\n\n"
                        f"📝 {restored_count} file(s) restored\n"
                        f"📋 Plan was NOT modified (--code-only mode).\n"
                        f"Chat history was NOT modified (--code-only mode)."
                    )
                )
            else:
                await self._mount_and_scroll(
                    UserCommandMessage(
                        f"Restored to snapshot {message_index}\n\n"
                        f"{restored_count} file(s) restored. "
                        f"Chat history truncated to message {message_index}. "
                        f"{cleanup_msg}"
                        f"{plan_msg}"
                        f"You can now edit the question and resubmit, or continue with a new message."
                    )
                )
    ...
    async def _undo_last(self) -> None:
        """Undo last change by restoring to previous snapshot."""
        if self.agent is None or not self.agent.snapshot_manager:
            await self._mount_and_scroll(
                ErrorMessage(
                    "Snapshots not available.",
        ...
        if len(snapshots) < 2:
            await self._mount_and_scroll(
                UserCommandMessage(
                    "No previous messages to undo to."
                )
            )
            return

        # The last snapshot captures the state *after* the most recent turn,
        # so restoring to it is a no-op. Use the second-to-last snapshot
        # to revert to the state *before* the last turn.
        target_snapshot = snapshots[-2]

        # Set the command input to simulate "/restore N" and call restore
        self._last_command_input = f"/restore {target_snapshot.message_index}"
        await self._restore_snapshot()
        #### KK-code altercation END ####

    #### KK-code altercation BEGIN ####
    async def _show_sessions(self) -> None:
        """Show interactive session selector in bottom panel."""
        from vibe.core.interaction_logger import InteractionLogger

        sessions = InteractionLogger.list_sessions(
            self.config.session_logging,
            self.config.effective_workdir
        )

        if not sessions:
            await self._mount_and_scroll(
                UserCommandMessage("No previous sessions available.")
            )
            return

        await self._switch_to_session_selector(sessions)
    #### KK-code altercation END ####
    ...
    #### KK-code altercation BEGIN ####
    async def _switch_to_snapshot_selector(
        self, snapshots: list, current_index: int
    ) -> None:
        """Switch to snapshot selector in the bottom panel."""
        bottom_container = self.query_one("#bottom-app-container")
        ...
        selector = SnapshotSelector(snapshots, current_index)
        await bottom_container.mount(selector)
        self._current_bottom_app = BottomApp.Snapshot

        self.call_after_refresh(selector.focus)
        self.call_after_refresh(self._scroll_to_bottom)

    async def _switch_to_session_selector(self, sessions: list) -> None:
        """Switch to session selector in the bottom panel."""
        bottom_container = self.query_one("#bottom-app-container")
    ...
    async def _switch_to_input_app(self) -> None:
        ...
        #### KK-code altercation BEGIN ####
        try:
            snapshot_selector = self.query_one("#snapshot-selector")
            await snapshot_selector.remove()
        except Exception:
            pass

        try:
            session_selector = self.query_one("#session-selector")
            await session_selector.remove()
        except Exception:
            pass
        #### KK-code altercation END ####
    ...
    def _focus_current_bottom_app(self) -> None:
        try:
            match self._current_bottom_app:
                ...
                case BottomApp.Snapshot:  #### KK-code altercation
                    self.query_one(SnapshotSelector).focus()  #### KK-code altercation
                case BottomApp.Session:  #### KK-code altercation
                    self.query_one(SessionSelector).focus()  #### KK-code altercation
        except Exception:
            pass
    ...
    def action_interrupt(self) -> None:
        ...
        #### KK-code altercation BEGIN ####
        if self._current_bottom_app == BottomApp.Snapshot:
            try:
                snapshot_selector = self.query_one(SnapshotSelector)
                snapshot_selector.action_cancel()
            except Exception:
                pass
            self._last_escape_time = None
            return
        ...
        if self._current_bottom_app == BottomApp.Session:
            try:
                session_selector = self.query_one(SessionSelector)
                session_selector.action_cancel()
            except Exception:
                pass
            self._last_escape_time = None
            return
    ...
    def _switch_mode(self, mode: AgentMode) -> None:
        ...
        if self.agent:
            if mode.auto_approve:
                ...
                # Wrap the approval callback to show preview diffs before approval
                self.agent.approval_callback = self._create_preview_approval_callback()
        #### KK-code altercation END #####
...
def _print_session_resume_message(session_id: str | None) -> None:
    ...
    print()
    print("To continue this session, run: kkcode --continue")
    print(f"Or: kkcode --resume {session_id}")
...
def run_textual_ui(
    ...
    initial_prompt: str | None = None,
    loaded_messages: list[LLMMessage] | None = None,
    resumed_session_id: str | None = None,  #### KK-code altercation: Pass session ID when resuming
) -> None:
    ...
    app = KKCodeApp(
        ...
        resumed_session_id=resumed_session_id,  #### KK-code altercation
