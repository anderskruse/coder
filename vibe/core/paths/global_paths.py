...
_DEFAULT_VIBE_HOME = Path.home() / ".kkcode"  #### KK-code altercation


def _get_vibe_home() -> Path:
    #### KK-code altercation BEGIN ####
    # Support both KKCODE_HOME and VIBE_HOME for backwards compatibility
    if kkcode_home := os.getenv("KKCODE_HOME"):
        return Path(kkcode_home).expanduser().resolve()
