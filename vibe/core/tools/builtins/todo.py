...
class TodoArgs(BaseModel):
    ...
    @classmethod
    def parse_todos_if_string(cls, v: Any) -> Any:
        """Parse todos if it's provided as a JSON string instead of a list."""
        if isinstance(v, str):
            try:
                parsed = json.loads(v)
                # If it parsed successfully, return the parsed value
                return parsed
            except (json.JSONDecodeError, TypeError):
                # If parsing fails, let Pydantic handle the validation error
                pass
        return v
