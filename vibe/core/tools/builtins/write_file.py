...
class WriteFile(
    ...
    @final
    async def get_preview(self, args: WriteFileArgs) -> tuple[Path, str] | None:
        """Generate preview of what the file will look like after writing.

        This is used for showing diffs before approval. Returns (file_path, preview_content)
        or None if preview cannot be generated.
        """
        try:
            # Validate the file path and check if we can write (same validation as run())
            file_path, file_existed, _ = self._prepare_and_validate_path(args)

            # Return the file path and the content that will be written
